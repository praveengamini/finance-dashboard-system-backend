...
from records.schemas import RecordCreate, RecordResponse, RecordUpdate, RecordListResponse
...
async def list_records_endpoint(
    ...
    """Admin and analyst. Filter by type, category, date range, or search."""
    return await get_records(db, type, category, date_from, date_to, search, page, page_size)
...
async def list_deleted_records_endpoint(
    db: AsyncSession = Depends(get_db),
    user: dict = Depends(require_roles(["admin"])),
):
    """Admin only. View all soft-deleted records."""
    return await get_deleted_records(db)
...
async def get_record_endpoint(
    ...
    """Admin and analyst. Fetch one record by UUID."""
